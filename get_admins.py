...
def get_command(URL_to_use, sys_id):
    #generate command with sys id
    return f"curl -H \"Authorization: Bearer {token}\" \"{URL_to_use[0]}{sys_id}{URL_to_use[1]}\" >> {input_filename}"
...
def write_to_file(URL_to_use,sys_IDs, out = input_filename):
    ...
    num_users = len(sys_IDs) 
    for n in sys_IDs:
        os.system(get_command(URL_to_use, n))
        if n != sys_IDs[-1]:
            os.system(f"echo \",\" >> {out}")    
...
def main():
    ...
    write_to_file(URL_LOGIN, sys_IDs, out = directory + login_path)
    fix_json(input_filename = login_path)
